...
/**
 * KhuvucPhuctapAnNinhSearch represents the model behind the search form about `app\modules\quanly\models\KhuvucPhuctapAnNinh`.
 */
class KhuvucPhuctapAnNinhSearch extends KhuvucPhuctapAnNinh
{
    /**
     * @inheritdoc
     */
    public function rules()
    {
        return [
            [['id', 'status'], 'integer'],
            [['ten', 'loai_khuvuc', 'muc_do_phuctap', 'phuong_xa', 'quan_huyen', 'ghi_chu', 'geom', 'created_at', 'created_by', 'updated_at', 'updated_by'], 'safe'],
            [['lat', 'long'], 'number'],
        ];
    }
    ...
    {
        $query = KhuvucPhuctapAnNinh::find()->where(['status' => 1]);
        ...
        $query->andFilterWhere(['like', 'upper(ten)', mb_strtoupper($this->ten)])
            ->andFilterWhere(['like', 'upper(loai_khuvuc)', mb_strtoupper($this->loai_khuvuc)])
            ->andFilterWhere(['like', 'upper(muc_do_phuctap)', mb_strtoupper($this->muc_do_phuctap)])
