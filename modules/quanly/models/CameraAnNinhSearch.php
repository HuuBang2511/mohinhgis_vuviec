...
/**
 * CameraAnNinhSearch represents the model behind the search form about `app\modules\quanly\models\CameraAnNinh`.
 */
class CameraAnNinhSearch extends CameraAnNinh
{
    /**
     * @inheritdoc
     */
    public function rules()
    {
        return [
            [['id', 'status'], 'integer'],
            [['ma_camera', 'ten_diem', 'dia_chi', 'phuong_xa', 'quan_huyen', 'don_vi_quan_ly', 'trang_thai', 'nguon_du_lieu', 'geom', 'created_at', 'created_by', 'updated_at', 'updated_by'], 'safe'],
            [['lat', 'long'], 'number'],
        ];
    }
    ...
    {
        $query = CameraAnNinh::find()->where(['status' => 1]);
        ...
        $query->andFilterWhere(['like', 'upper(ma_camera)', mb_strtoupper($this->ma_camera)])
            ...
            ->andFilterWhere(['like', 'upper(trang_thai)', mb_strtoupper($this->trang_thai)])
            ->andFilterWhere(['like', 'upper(nguon_du_lieu)', mb_strtoupper($this->nguon_du_lieu)])
